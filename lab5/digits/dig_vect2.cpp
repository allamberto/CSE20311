...
void findfreq(vector<int> a, vector<int>& freq)
{
      for (auto it = a.begin(); it < a.end(); it++) //loop through entire vector checking for digit at each iteration
                         (freq[*it])++; // add one to each position of freq that the position of a falls under
}

void results(vector<int> a, vector<int> freq)
{
        for (auto it = freq.begin(); it < freq.end(); it++)
                cout <<  "digit " << it - freq.begin() << " occurs " << *it << " times" << endl; //it - freq.begin returns current position in vector
}
